            ...
            {items.map(item => {
              ...
              return (
                <motion.div key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 12, background: item.active ? 'var(--bg-tertiary)' : 'var(--bg-primary)', border: '1px solid var(--border-color)', opacity: item.active ? 1 : 0.5 }}>
                  ...
                  <span style={{ fontWeight: 700, color: 'var(--expense)', fontSize: 15, flexShrink: 0 }}>
                  ...
                  <button onClick={() => removeItem(item.id)} style={{ color: 'var(--danger)', padding: 4 }}><Trash2 size={15} /></button>
                </motion.div>
              );
            })}
          </AnimatePresence>
        ...
        {adding ? (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setAdding(false)} style={{ flex: 1, padding: '10px', borderRadius: 10, background: 'var(--bg-tertiary)', color: 'var(--text-secondary)', fontWeight: 600 }}>취소</button>
                <button onClick={addItem} style={{ flex: 2, padding: '10px', borderRadius: 10, background: 'var(--accent)', color: 'white', fontWeight: 700 }}>추가</button>
              </div>
            </div>
          </motion.div>
        ) : (
...
const inp = { width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid var(--border-color)', background: 'var(--bg-tertiary)', color: 'var(--text-primary)', fontSize: 14 };
